...
In other words in expectation of the ensemble average is consistent with the
...
For large but finite $N$ the higher-order cumulants of the ensemble average
don't vanish exactly, but they are still overshadowed by the mean.  When $N$ is
reasonably large the pushforward distribution is approximately characterized by
only its mean and variance.  More formally the pushforward probability density
function is well-approximated by a normal probability density function
$$
...
$$
This intermediate approximation is known as a _central limit theorem_.

As the ensemble size $N$ grows the variance narrows and this normal
approximation converges to the limiting Dirac distribution.  In other words
the law of large numbers determines to _what_ the pushforward distribution along
the ensemble average will converge, but the central limit theorem quantifies
_how_ it converges.  The how is far more useful in practical circumstances where
infinity is just out of reach.
...
$f : X \rightarrow \mathbb{R}$ then the element defines a sequence that is
_asymptotically consistent_ with the base probability distribution.
...
hope that a sample will contain enough information to _approximate_ $\pi$.
...
q(y) = \Pi^{-1}(y),
$$
maps the uniform distribution over the unit real interval $\upsilon$ back to the
...
Preasymptotically consistent sequences allow us not only approximate expectation
values with ensemble averages over only finite samples but also quantify the
error of that approximation with the pushforward distribution along the ensemble
average function.  The _Monte Carlo method_ appeals to the central limit theorem
to approximate that pushforward distribution and, consequently, this error
quantification.
...
\frac{ (\left< f \right>_{1:N})_{*} \pi_{1:N}(y) - \mathbb{E}_{\pi}[f] }
{ \sqrt{ \mathbb{V}_{\pi}[f] / N } }
+ \mathcal{O}(N^{-2}) = \text{normal}(0, 1).
$$
For any fixed $N$ we can rearrange this to give the approximate probability
density function for the pushforward distribution,
$$
(\left< f \right>_{1:N})_{*} \pi_{1:N}(y)
=
\text{normal} \left( y \mid \mathbb{E}_{\pi}[f], \sqrt{ \frac{\mathbb{V}_{\pi}[f]}{N} } \right)
...
how large of a sample we need to achieve a given error.  For example if we want
a relative error of at most $\alpha$ then we need a sample size of at least
$$
\begin{align*}
\alpha &\ge \frac{ \hat{\sigma}[f]  }{ \hat{\mu}[f]  }
\\
...
standard error $\hat{\sigma}[f](x_{1}, \ldots, x_{N}) / \sqrt{N}$.
...
$$
\begin{align*}
B_{1} &= (\beta_{0}, \beta_{1}),
\\
B_{2} &= (\beta_{1}, \beta_{2}),
\\
&\ldots,
\\
B_{K}(\beta_{K - 1}, \beta_{K}),
$$
...
so that those infinitesimally narrow bins would could be occupied!  Moreover
...
Let's put all of this into practice with some explicit demonstrations.
...
### Multidimensional Target Distribution
...
Once again we have consistency with the exact value, in this case $\sigma = 1$.
...
When moments or quantiles are not all that well-behaved quantiles are often more
...
uniformly than random numbers such that ensemble averages over _contiguous_
...
Bryan Yu, Brynjolfur Gauti Jónsson, Cameron Smith, Canaan Breiss, Cat Shark, 
...
Kevin Thompson, Kádár András, lizzie , Luiz Carvalho, Luiz Pessoa, Marc Dotson, 
Marc Trunjer Kusk Nielsen, Marek Kwiatkowski, Mario Becerra, Mark Donoghoe, 
Mark Worrall, Markus P., Martin Modrák, Matthew Hawthorne, Matthew Kay, 
...
Ryan Grossman, Rémi , Scott Block, Scott Treloar, Sean Talts, Serena, Seth Axen, 
...
Stone Chen, Sus, Susan Holmes, Suyog Chandramouli, Sören Berg, Tao Ye, tardisinc, 
