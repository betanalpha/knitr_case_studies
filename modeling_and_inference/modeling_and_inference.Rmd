...
Given a parameterization we can then specify the observational model with a family 
...
generating processes captures the structure of the true data 
...
Formally these likelihood evaluations provide only _relative_ information -- 
...
which can lead to confusion in practice because Bayes' Theorem is a general result
...
Moreover, closed-form posterior density functions rely on very simple models and
...
$$
a^{*}(\tilde{y}) = \underset{a \in A}{\text{argmax}} \overline{U}(a, \tilde{y}).
$$
...
pathological behavior that can compromise our inferences.  The ideal behavior
...
model then we have to rely on the general frequentist and Bayesian methodologies.
