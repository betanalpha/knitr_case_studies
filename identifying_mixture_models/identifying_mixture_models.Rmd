...
\end{align*}
$$
and then we exploit the permutation-invariance of the function of the integrand
to give,
$$
\begin{align*}
\mathbb{E}_{\pi} [ f ]
...
$$
\pi(y_1, \ldots, y_N \mid \mu_1, \sigma1, \mu_2, \sigma2, \theta_1, \theta_2)
=
sum_{n = 1}^{N}
...
weights, let's try to fix conditioned on a single mixture weight,
```{r, cache=TRUE}
theta <- 0.25
...
```
Still, care must be taken in practice to ensure that the posterior geometry does
not become too pathologic for Stan to accurately fit.

In order to avoid the bowtie geometry entirely we need to ensure that the
components are sufficiently separated.  If this such separation is not induced
