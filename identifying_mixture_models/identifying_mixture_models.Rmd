...
```{r, comment=NA}
check_all_diagnostics(degenerate_fit)
```
...
```{r, comment=NA}
check_all_diagnostics(asym_fit)
```
...
```{r, comment=NA}
check_all_diagnostics(ordered_fit)
```
...
effective samples over the 4000 total iterations for the component means,
```{r, comment=NA}
summary(singular_fit, probs = c(0.5))$summary[,5]
```
...
```{r, comment=NA}
check_all_diagnostics(singular_fit)
```
