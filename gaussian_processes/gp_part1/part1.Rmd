...
closer less their functional outputs, $f(x_1)$ and $f(x_2)$, can vary.
...
as covariance functions form an _algebra_.  Not only is the sum of two
covariance functions also a covariance function, so too is their product.  
Consequently we can build quite sophisticated Gaussian processes by
exploiting just a few base covariance functions.
...
it referred to as a squared exponential kernel amonst other names.
...
mean and covariance funtions.  The posterior Gaussian process
...
sampled function.  Note that we add a _nugget_ of $10^{-10}$ so the
marginal covariance matrix before computing its Cholesky decomposition
