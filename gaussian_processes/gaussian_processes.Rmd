...
covariance function by plotting its values verses distance in a _variogram_,
or its normalized values verse distance in a _correlogram_.

<center>
<br>
```{r, out.width = "100%", echo=FALSE}
knitr::include_graphics("figures/variogram_general/variogram_general.png")
```
<br><br>
</center>
...
infinitely easier -- literally -- if we consider not all function values but
rather only those at a finite number of covariate values.  I will refer to the
finite number of covariate values of interest as a _grid_.  In this case the
function values over the grid follow a distribution specified by a multivariate
normal density function completely determined by the Gaussian process.
...
\mathbf{K} =
\begin{pmatrix}
\mathbf{K}_{\text{obs}} & \mathbf{K}_{\text{pred}} \\
( \mathbf{K}_{\text{pred}} )^{T} & \mathbf{K}_{\text{pred}}
\end{pmatrix}.
$$

The conditional probability density function for the unobserved function values
given the observed function values $f_{n} = f(x^{\text{obs}}_{n})$ is then given
by the location parameters
$$
\boldsymbol{\mu}
=
\mathbf{m}_{\text{pred}}
+ ( \mathbf{K}_{\text{mix}} )^{T} \cdot ( \mathbf{K}_{\text{obs}} )^{-1}
+ \cdot (\mathbf{f} - \mathbf{m}_{\text{obs}})
$$
...
Now that we've worked out how to implement Gaussian processes in practice we can
also start to see their practical limitations.

For example because the projection of a Gaussian process results in a
multivariate normal density function that correlates _all_ of the function
values at the grid points with each other we cannot add new grid points without
constructing an entirely new multivariate normal density function and repeating
all of the subsequent linear algebra.  In other words Gaussian processes are
awkward when we can't specify the relevant covariate values ahead of time.
...
$$
k(x_{1}, x_{2}) = \alpha^{2}
\exp \left( - \left( \frac{ |x_{1} - x_{2} |}{\rho} \right)^{2} \right).
$$

<center>
<br>
```{r, out.width = "75%", echo=FALSE}
knitr::include_graphics("figures/variogram_exp_quad/variogram_exp_quad.png")
```
<br><br>
</center>
...
k(x_{1}, x_{1})
&= \alpha^{2}
\exp \left( - \left( \frac{ |x_{1} - x_{1} |}{\rho} \right)^{2} \right)
\\
&= \alpha^{2}
\exp \left( - \left( \frac{0}{\rho} \right)^{2} \right)
\\
&= \alpha^{2}
\exp \left( - 0 \right)
\\
...
varied from small to large values.
...
behavior _at_ the observed covariate values while the structure of the covariance
function constrains how that behavior propagates _between_ the observed
covariate values.
...
I thank Dan Simpson, Aki Vehtari, and Rob Trangucci for helpful comments.
