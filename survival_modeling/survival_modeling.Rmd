...
\tilde{p} &\sim \text{uniform}(0, 1)
\\
t &= S^{-1}(\tilde{p}) = - \frac{ \log \tilde{p} }{ \gamma }.
\end{align*}
$$
...
t 
&= S^{-1}(\tilde{p}) 
= - \sigma \, \left(- \log \tilde{p} \right)^{ \frac{1}{\alpha} }.
\end{align*}
$$
...
plot_marginal("gamma2", surv_samples, gamma2, c(0, 1.5))
```

The shoulder in these marginal posterior distributions is a consequence of an
inherent multiplicative degeneracy between $\gamma_{1}$ and $\gamma_{2}$.  The
larger $\gamma_{1}$ the more quickly the precursor event will tend to occur; in
order to fit the observed event times the subsequent events will then have to
take longer, resulting in a smaller $\gamma_{2}$.  Likewise if the precursor
events take longer then the subsequent events will have to occur more quickly in 
order to maintain consistency with the observations.

```{r}
par(mar = c(5, 4, 3, 2))
par(mfrow=c(1, 1))

plot(surv_samples$gamma1, surv_samples$gamma2, pch=16, cex=1, 
     col=c_dark, main="",
     xlab="gamma1", xlim=c(0, 1), ylab="gamma2", ylim=c(0, 1.5))
points(gamma1, gamma2, pch=16, cex=1.5, col="white")
points(gamma1, gamma2, pch=16, cex=1.0, col="black")
```

Regardless the posterior retrodictive checks demonstrate no systematic deviation
...
As expected the behavior of the marginal posterior distributions is equivalent
to that of our initial fit.
