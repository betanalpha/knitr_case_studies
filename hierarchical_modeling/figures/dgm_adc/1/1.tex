...
\draw[white] (-25, -13) rectangle (24, 16);
...
\draw[->, >=stealth, color=mid, line width=1.5] (-5, -10) -- ({0 - \r * cos(45)}, {0 - \r * sin(45)});

\filldraw[fill=white, draw=dark, line width=1.5] (-5, -10) circle (\r)
node[color=black] { $\log \psi$ };

\node[] at (17, 7)    { $\tilde{y}_{k,n} \sim \text{Poisson}(\lambda_{k})$ };
\node[] at (17, 0)    { $\lambda_{k} = \exp(\log \psi) \cdot \rho_{I, k}$ };

\node[] at (-17, -10)  { $\log \psi \sim \text{normal}(0, \log 10)$ };
