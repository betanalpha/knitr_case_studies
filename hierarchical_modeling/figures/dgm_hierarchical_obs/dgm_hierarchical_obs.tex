...
\draw[white] (-21 + \dx, -7 + \dy) rectangle (21 + \dx, 13 + \dy);

%\node[align=center] at (0 + \dx, -8 + \dy) 
%{ $ \pi(y_{1}, \theta_{1}, \ldots, y_{k}, \theta_{k}, \ldots, y_{K}, \theta_{K}, \phi) = \left[ \prod_{n = 1}^{N} \pi(y_{k} \mid \theta_{k} ) \cdot \pi(\theta_{k} \mid \phi) \right] \cdot \pi(\phi) $ };

\filldraw[fill=dark, draw=dark, line width=1.5] (-12 + \dx, 9 + \dy) circle (\r)
node[color=white] { $\tilde{y}_{1}$ };
