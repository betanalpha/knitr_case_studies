...
\draw[white] (-21 + \dx, -10 + \dy) rectangle (21 + \dx, 12 + \dy);

\node[align=center] at (0 + \dx, -8 + \dy) 
{ $ \pi(y, \theta_{n}, \phi) = \pi(y \mid \theta_{1}, \ldots, \theta_{n}) \cdot \left[ \prod_{n = 1}^{N} \pi(\theta_{n} \mid \phi) \right] \cdot \pi(\phi) $ };

\filldraw[fill=dark, draw=dark, line width=1.5] (-6 + \dx, 9 + \dy) circle (\r)
node[color=white] { $y_{n}$ };

\draw[->, >=stealth, color=mid, line width=1.5] (-12 + \dx, 3 + \r + \dy) -- ({-6 - \r * cos(45) + \dx}, {9 - \r * cos(45) + \dy});
\draw[->, >=stealth, color=mid, line width=1.5] (-6 + \dx, 3 + \r + \dy) -- (-6 + \dx, 9 - \r + \dy);
\draw[->, >=stealth, color=mid, line width=1.5] (0 + \dx, 3 + \r + \dy) -- ({-6 + \r * cos(45) + \dx}, {9 - \r * cos(45) + \dy});
