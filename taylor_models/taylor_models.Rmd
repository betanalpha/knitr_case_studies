...
how large the perturbations $x - x_{0}$ can be, and hence the large the powers
$(x - x_{0})^{i}$ from which Taylor approximations are constructed can be.
Generally the smaller the neighborhood the more quickly these powers will decay
and the smaller the Taylor remainder will be across $U$.  In other words
...
smaller the the more accurate a given Taylor approximation will be; the
...
we associate Jacobian arrays with tensors_.  In other words we have to first
fix the input space first before we fill up the Jacobian arrays with partial 
derivative evaluations in that parameterization and then apply tensor operations 
to those arrays.  Anytime we want to reparameterize the input space we have to 
redo the _entire_ construction.  
...
We can verify this with a little bit of multivariable calculus.  If an finite
...
neighborhood.  In particular the derivative evaluations have to not yield not
only unique values but also finite values.  Unfortunately this can be a problem 
for functions that are defined on only a subset of the input space.
...
implies that all of the derivatives are ill-defined at $x = 0$.  Consequently 
we cannot construct Taylor approximations from a base points right on the 
boundary $x_{0} = 0$.  
...
two key assumptions.  First we assume that here is no confounding behavior that 
...
Critically this interpretation depends the chosen baseline; changing $x_{0}$ 
...
\end{align*}
$$
provided that keep the parameterization of the covariate space fixed.
...
be interpretable local differential structure at the baseline.
...
regression model. If the covariates from incomplete observations concentrates in 
...
vector organizes the repeated evaluations of teh entire $I$-th order Taylor 
...
the _parameters_.  In much many fields this latter notion of linearity is
prioritized so that the term "linear model" refers to _any_ regression model 
that can be implemented with a single product of a fixed matrix and a vector of
parameters, regardless of how that matrix is constructed.
...
then be visualized with, for example, ribbon plots or marginal quantile plots.
...
plot_realizations(x_grid, samples$f_grid,
                  "Prior Realizations",
                  "x", "mu", c(-3.5, 3.5), c(-15, 15))
abline(v=x0, lwd=2, col="grey", lty=3)

plot_marginal_quantiles(x_grid, samples$f_grid,
...
```{r}
plot_realizations(data$x_grid, samples$f_grid, "Posterior Realizations",
...
```{r}
plot_marginal_quantiles(data$x_grid, samples$f_grid, "Posterior Marginal Quantiles",
                        "x", "y", c(-3.5, 3.5), c(-28, 7))
...
plot_realizations(data$x_grid, samples1$f_grid,
...
plot_realizations(data$x_grid, samples2$f_grid,
                  "Posterior Realizations",
                  "x", "y", c(-3.5, 3.5), c(-28, 7))
...
```{r}
plot_marginal_quantiles(data$x_grid, samples2$f_grid,
                        "Posterior Marginal Quantiles",
                        "x", "y", c(-3.5, 3.5), c(-28, 7))
...
<details>
<summary>Click for definition of `marginal_mu`</summary>
```{r}
...
```{r}
par(mfrow=c(1, 1), mar = c(5, 5, 3, 1))
marginal_location(samples$f, -200, 200, 25)
```
...
```{r}
conditional_location <- function(obs_covar, f, name, bin_min, bin_max, B) {
  breaks <- seq(bin_min, bin_max, (bin_max - bin_min) / B)
  idx <- rep(1:B, each=2)
...
conditional_location(data$X[,1], samples$f, "x1", -7, 11, 20)
abline(v=data$x0[1], lwd=2, col="grey", lty=3)
conditional_location(data$X[,2], samples$f, "x2", -7, 11, 20)
abline(v=data$x0[2], lwd=2, col="grey", lty=3)
conditional_location(data$X[,3], samples$f, "x3", -7, 11, 20)
...
conditional_location(data$X[,1], samples$f, "x1", -6, 9, 20)
abline(v=data$x0[1], lwd=2, col="grey", lty=3)
conditional_location(data$X[,2], samples$f, "x2", -6, 9, 20)
abline(v=data$x0[2], lwd=2, col="grey", lty=3)
conditional_location(data$X[,3], samples$f, "x3", -6, 9, 20)
abline(v=data$x0[3], lwd=2, col="grey", lty=3)
```
...
conditional_location(data$X[,1], samples$f, "x1", -8, 8, 20)
abline(v=data$x0[1], lwd=2, col="grey", lty=3)

conditional_location(data$X[,2], samples$f, "x2", -8, 8, 20)
abline(v=data$x0[2], lwd=2, col="grey", lty=3)

conditional_location(data$X[,3], samples$f, "x3", -8, 8, 20)
abline(v=data$x0[3], lwd=2, col="grey", lty=3)
```
...
conditional_location(data$X[,1], samples$f, "x1", -9, 9, 20)
abline(v=data$x0[1], lwd=2, col="grey", lty=3)
conditional_location(data$X[,2], samples$f, "x2", -9, 9, 20)
abline(v=data$x0[2], lwd=2, col="grey", lty=3)
conditional_location(data$X[,3], samples$f, "x3", -9, 9, 20)
abline(v=data$x0[3], lwd=2, col="grey", lty=3)
```
...
```{r}
par(mfrow=c(1, 1), mar = c(5, 5, 3, 1))
marginal_location(samples$f, -300, 300, 25)
```
...
```{r}
par(mfrow=c(1, 1), mar = c(5, 5, 3, 1))
marginal_location(samples$f, -1000, 7000, 50)
```

```{r}
par(mfrow=c(1, 3), mar = c(5, 5, 2, 1))

conditional_location(data$X[,1], samples$f, "Temp (oF)", 65, 95, 20)
abline(v=data$x0[1], lwd=2, col="grey", lty=3)
conditional_location(data$X[,2], samples$f, "Price (c$)", 250, 350, 20)
abline(v=data$x0[2], lwd=2, col="grey", lty=3)
conditional_location(data$X[,3], samples$f, "Bookings", 200, 2200, 20)
abline(v=data$x0[3], lwd=2, col="grey", lty=3)
```
...
```{r}
par(mfrow=c(1, 1), mar = c(5, 5, 3, 1))
marginal_location(samples$f, -3000, 9000, 50)
```
