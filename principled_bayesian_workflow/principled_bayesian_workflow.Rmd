...
configuration space,
$$
z[f \mid \tilde{y}, \theta^{\dagger}] = \left|
\frac{ \mathbb{E}_{\mathrm{post}}[f \mid \tilde{y}] - f(\theta^{\dagger}) }
{ \mathbb{E}_{\mathrm{post}}[f \mid \tilde{y} ] } \right|,
$$
...
$$
\hat{\delta}(\pi^{\text{pred}}) \! \left( \pi \mid\mid \pi^{\text{pred}} \right)
\equiv
...
```{r}
writeLines(readLines("prior_tune1.stan"))
```

```{r}
stan(file='prior_tune1.stan', iter=1, warmup=0, chains=1,
     seed=4838282, algorithm="Fixed_param")
```

To simplify copy and pasting we'll take the scale 5.82337, although given the
approximate nature of this calculation there's really no reason to be keeping
all of those decimal places!

```{r}
lambda <- seq(0, 20, 0.001)

plot(lambda, dnorm(lambda, 0, 5.82337), type="l", col=c_dark_highlight, lwd=2,
...
\cdot \text{normal} (\lambda \mid 0, 5.82337).
$$
...
```{r}
writeLines(readLines("simu_bayesian_ensemble.stan"))
```
...
fit <- stan(file='simu_bayesian_ensemble.stan', data=simu_data,
...
respectively.  This visualization can obscure multimodality within a bin and
correlations between the bins, but it still communicates a tremendous amount of
information about the prior predictive distribution.
...
if (sum(warning_code) != 0) {
  ...
  print ("No posterior fits in the Bayesian ensemble encountered problems!")
}
```
...
```{r}
writeLines(readLines("fit_data_post_pred.stan"))
```

```{r}
fit <- stan(file='fit_data_post_pred.stan', data=input_data,
...
=
\theta \cdot \text{Poisson} (y_{n} \mid \lambda)
+ (1 - \theta) \cdot \delta_{0}(y_{n}) .
$$
We use the same prior for the source intensity, $\lambda$, and assign a uniform
prior over the mixture weight, $\theta$, to give the complete Bayesian model
$$
\begin{align*}
\pi( y_{1}, \ldots, y_{N}, \lambda )
&= \;\;
\prod_{n = 1}^{N} \Big[
\theta \cdot \mathrm{Poisson} (y_{n} \mid \lambda) + (1 - \theta) \cdot \delta_{0}(y_{n})
\Big]
\\
& \quad \cdot \text{normal} (\lambda \mid 0, 5.82337)
\\
& \quad \cdot
...
```{r}
writeLines(readLines("simu_bayesian_ensemble2.stan"))
```
...
fit <- stan(file='simu_bayesian_ensemble2.stan', data=simu_data,
            iter=R, warmup=0, chains=1, refresh=R,
            seed=4838282, algorithm="Fixed_param")
...
It should, because this is a manifestation of a _poor identifiability_ hiding in
our zero-inflated model.  When the Poisson source strength is small enough the
resulting distribution of counts will be indistinguishable from the Dirac
distribution that we are using to model the excess zeroes.

While we could try to reconfigure our computational method to better handle the
poorly identified likelihood functions, its manifestation also provides us with
an opportunity to reconsider our domain expertise and tweak our model.  Is it
reasonable for the source intensity to be small enough that we would expect
mostly zeroes from the Poisson model?
...
or not working?  The the above computational problems identified some important
...
```{r}
writeLines(readLines("prior_tune2a.stan"))
```

```{r}
stan(file='prior_tune2a.stan', iter=1, warmup=0, chains=1,
     seed=4838282, algorithm="Fixed_param")
...
```{r}
writeLines(readLines("prior_tune2b.stan"))
```

```{r}
stan(file='prior_tune2b.stan', iter=1, warmup=0, chains=1,
     seed=4838282, algorithm="Fixed_param")
...
\Big]
\\
& \quad \cdot \text{inv-gamma} (\lambda \mid 3.48681, 9.21604)
\\
& \quad \cdot
\text{beta} (\theta \mid 2.8663, 2.8663)
\end{align*}
$$
...
```{r}
writeLines(readLines("simu_bayesian_ensemble3.stan"))
```
...
fit <- stan(file='simu_bayesian_ensemble3.stan', data=simu_data,
            iter=R, warmup=0, chains=1, refresh=R,
            seed=4838282, algorithm="Fixed_param")
...
```{r}
writeLines(readLines("fit_data3_post_pred.stan"))
```

```{r}
input_data <- read_rdump('workflow.data.R')
fit <- stan(file='fit_data3_post_pred.stan', data=input_data,
            seed=4938483, refresh=2000)
```
...
observation.  That model, however, is a bit ungainly so for pedagogical
...
```{r}
writeLines(readLines("simu_bayesian_ensemble4.stan"))
```
...
fit <- stan(file='simu_bayesian_ensemble4.stan', data=simu_data,
            iter=R, warmup=0, chains=1, refresh=R,
            seed=4838282, algorithm="Fixed_param")
...
```{r}
writeLines(readLines("fit_data4_post_pred.stan"))
```

```{r}
input_data <- read_rdump('workflow.data.R')
fit <- stan(file='fit_data4_post_pred.stan', data=input_data,
            seed=4938483, refresh=2000)
```
...
Desislava Petkova, arabidopsis, Shira Mitchell, Aki Vehtari, and Adam
Fleischhacker for helpful comments as well as the feedback from those who have
attended my courses and talks.
