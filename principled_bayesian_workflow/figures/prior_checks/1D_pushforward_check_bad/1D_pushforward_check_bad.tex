...
declare function={ g(\x) = exp(-0.5 * (\x - 0) * (\x - 0) / (2 * 2)) / (2.506628274631001 * 2);}
  ...
  \fill[color=gray80, opacity=0.33] (-10, 0) rectangle (-2.5, 10.5);
  \draw[-, color=gray70, line width=1] (-2.5, 0) -- (-2.5, 10.5);
  \draw[->, >=stealth, line width=1, color=gray70] (-2.5, 5) -- +(-1.5, 0);

  \fill[color=gray90, opacity=0.33] (10, 0) rectangle (2.5, 10.5);
  \draw[-, color=gray70, line width=1] (2.5, 0) -- (2.5, 10.5);
  \draw[->, >=stealth, line width=1, color=gray70] (2.5, 5) -- +(1.5, 0);
