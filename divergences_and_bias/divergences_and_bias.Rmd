...
ergodicity is typically the necessary condition for MCMC estimators to follow
a central limit theorem, which ensures not only that they are unbiased even for
finite Markov chains but also that we can empirically quantify their precision
using the MCMC standard error
...
initialized from diffuse points in parameter space; to do any better requires
exploiting the structure of a particular transition or target distribution.
...
of these behaviors are _divergences_ that indicate the Hamiltonian Markov chain
has encountered regions of high curvature in the target distribution which it
cannot explore.
...
Consider the Eight Schools hierarchical model [@Rubin:1981],
...
Eight Schools model and see how divergences identify this bias before there are
any other indications of problems.  We'll then use these divergences to study
the source of the bias and motivate potential fixes, first by adjusting the
adaptation in Stan and then by reimplementing the model with a _non-centered_
parameterization.  For a more thorough discussion of the geometry of centered
and non-centered parameterizations of hierarchical models see
@BetancourtEtAl:2015.
...
chain commonly motivated by computational expediency and then a longer Markov
chain.
...
use short chains when prototyping their analysis, or even for this final
...
squeezed together.  Indeed, this is exactly the funnel geometry infamous to
hierarchical models, except cut off at the neck where the Hamiltonian Markov
chain is unable to explore further and instead diverges.
...
encounters regions of extremely large curvature, such as the neck of the funnel
seen above.  We can control how strongly this curvature effects the Hamiltonian
Markov chain, however, by adjusting the step size, $\epsilon$.
...
With the increasing ```adapt_delta```, and decreasing step size,
```{r}
...
plot(adapt_delta, step_scan, xlab="Adept Delta", ylab="Adapted Step Size",
     xlim=c(0.79, 1.0), ylim=c(0, 0.2), col=c_dark, type="l", lwd=3)
points(adapt_delta, step_scan, col=c_dark, pch=16, cex=0.8)
```

we would expect the divergences to decrease, but counter-intuitively they remain
constant!
```{r}
...
deeper into the neck of the funnel but still ends up diverging when it probes
too far,
```{r}
...
That said, the superior exploration given by decreasing the step size does
reduce the bias of the resulting MCMC estimators,
```{r}
...
Although reducing the step size improves exploration, it only reveals the true
extent the pathology in the centered implementation.  Fortunately, there is
another way to implement hierarchical models that does not suffer from the same
pathologies.
...
we can recover the group-level parameters with a scaling and translation,
...
a non-centered implementation is not actually a reparamterization of the
centered implementation, but the terms "centered parameterization" and
"non-centered parameterization" have persisted in the literature and so I will
use them here.
...
Very rarely, however, we do still see a divergence,
```{r}
...
but interestingly they do not seem concentrate anywhere in parameter space,
```{r}
divergent <- get_sampler_params(fit_ncp80, inc_warmup=FALSE)[[1]][,'divergent__']
...
We can fix these scarce divergences by decreasing the step size,
```{r, cached=TRUE}
...
which finally gives us a long chain with no divergences,
```{r}
...
By analyzing the Eight Schools model we have seen that, when the data are sparse,
hierarchical models implemented with a centered parameterization impede the
geometric ergodicity of most Markov transitions.  The ultimate importance of
the resulting bias, however, depends on the application.  

If we are interested in the behavior of $\log \tau$ then this bias is huge and
will devastate to the quality of any inferences drawn from the fit.  Often,
however, one is not interested in the behavior of $\log \tau$ but rather the
behavior of $\tau$.  When we transform to the linear scale the pathological
region of the posterior is compressed into a small volume and the the biases
somewhat mediated,
```{r}
breaks=20 * (0:50) / 50
...
plot(p_ncp, col=c_mid, main="", xlab="tau")
plot(p_cp, col=c_dark_highlight, add=T)
...
At least the bias is not as obvious as it was on the logarithmic scale.

This mediation also affects how the bias propagates to other parameters in the
model.  When we average over the hierarchical prior, the small bias of $\tau$
the marginal posterior for the group-level parameters to be narrower than they
should be,
```{r}
...
This narrowness does not affect the accuracy of the mean of the group-level
parameters,
```{r}
...
but it does systematically underestimate the variance, at least for a long time,
```{r}
...
like Random Walk Metropolis and Gibbs samplers.
...
significant that bias will be.  The precise significance of any such bias depends
not only on the structure of the model but also on the details of how that model
will be applied in practice.  Sometimes an analysis taking these factors into
account can quantify the significance of the bias and potentially deem it
...
Regardless, divergences are an incredibly powerful diagnostic for the breakdown
of geometric ergodicity and hence critical guidance for best implementing our
models in practice.
