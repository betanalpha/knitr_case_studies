...
\begin{scope}[shift={(\x, \y)}, rotate={\phi}]
  \filldraw[fill=white, draw=dark, rounded corners=2] 
    (0 - \cx, 0 - \cy) rectangle (0 + \cx, 0 + \cy);
  \begin{scope}
    \clip (0 - \cx, 0 - \cy) rectangle (0 + \cx, 0 + \cy);
    \foreach \delta in {0, 0.05,..., 1} {
      \draw[dark] (0 - \cx, 0 - \cy - 2 * \cx + 6 * \delta * \cx) -- +(2 * \cx, 2 * \cx);
      \draw[dark] (0 - \cx, 0 - \cy + 6 * \delta * \cx) -- +(2 * \cx, -2 * \cx);
    }
  \end{scope}
\end{scope}
