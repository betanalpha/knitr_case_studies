...
contexts, but they become less appropriate when the contexts are not completely
exchangeable.  In particular any labels that categorize individual contexts into
groups immediately break exchangeability, and modeling heterogeneity
consistently with these groupings becomes much more subtle. When the contexts
are subject to multiple, overlapping categorizations the challenge becomes even
more difficult.  In this case study we investigate how to generalize
exchangeability in the presence of _factors_ that categorize individual contexts
and then develop modeling techniques compatible with this generalization.
...
We will denote this discriminating information with traditional statistics
terminology.  A _factor_ is a variable that endows each context with one of a
finite number of discrete, unordered values, or _levels_.  When there is minimal
risk of confusion, I will also use "level" to refer to all of the individual
contexts grouped with the same level value.  In our card example the immediate
factor is color, with the three levels corresponding to light red, red, and dark
red values.  The "dark red level" would then correspond to all cards with dark
red coloring.
...
The residual exchangeability that persists after a factor has been introduced
becomes more clear if we group the individual contexts by their level
assignments.  Because our domain expertise is invariant to permutations of the
contexts within each level we retain a _conditional exchangeability_.
...
This conditional exchangeability suggests that the heterogeneity of the
individual contexts is compatible with independent hierarchical models within
each level.
...
At the same time the level values themselves are also arbitrary labels, and any
model of behavior common to each level will be invariant to permutations of
these labels.  Consequently we also have a _marginal exchangeability_.
...
Consequently any parameters modeling that common behavior are naturally amenable
to a hierarchical model _across_ the levels.
...
## Factor Compatible Hierarchical Models
...
hierarchical model across the factor levels.  These hierarchies can also be
...
$$
\theta_{kl} \sim \pi(\phi_{l}),
$$
where $l \in \{1, \ldots, L\}$ indexes each level and
$k \in \{1, \ldots, K_{l} \}$ indexes the individual contexts within those
levels; the occupancy of each level, $K_{l}$, will not in general be uniform.
The full model specification is then completed with a population model across
the level-specific population parameters,
$$
\phi_{l} \sim \pi(\phi).
$$
...
$$
\theta_{kl} \sim \text{normal}(\theta_{l}, \tau_{l}).
$$
...
either case all of the individual contexts collapse into a single _level context_
for each factor level, and the remaining marginal heterogeneity across those
level contexts is captured by a single hierarchical model,
$$
\theta_{l} \sim \pi(\phi).
$$
...
parameters _relative to the location parameter_ then the hierarchical model
...
might be presented as the behavior in the _hypothetical_ circumstance where the
heterogeneity vanishes.  Although we might not observe this circumstance, and
indeed it might be impossible to observe this circumstance at all, it provides
_an_ interpretation for $\theta$ independent of the other parameters.  Similarly
the factor level residuals might be interpreted as the behavior within each
level in the hypothetical circumstance where the baseline contribution vanishes.

Personally I find these counterfactual interpretations confusing if not outright
misleading when they are made implicitly.  I recommend instead always using
_explicit_ configurations to interpret the residual parameters.  That
configuration might be _actual_, corresponding to levels observed in the data,
or _hypothetical_, corresponding to hypothetical levels with hypothetical
...
separating the full exchangeability into marginal exchangeability across the
...
symbol levels.  These symbol levels then motive their own own factor compatible
hierarchical models.
...
For example consider an initial factor that separates the individual contexts
into $I$ levels, and then a second factor that separates the contexts in the
$i$th level into $J_{i}$ sub-levels.  This nesting suggests a deeper hierarchy
that models heterogeneity across the $I$ levels and the marginal/conditional
heterogeneity across the $J_{i}$ sub-levels.  We will ignore any residual
conditional heterogeneity of the $K_{ij}$ contexts within each of those
sub-levels for now.
...
$$
\begin{align*}
\theta_{i} &\sim \text{normal}(\theta, \tau)
\\
\theta_{ij} &\sim \text{normal}(\theta_{i}, \tau_{i}).
\end{align*}
$$
...
_trivial factor_ that maps each context into its own level.
...
Consequently we can always add another hierarchy, one modeling the heterogeneity
compatible with the marginal exchangeability of the trivial levels, on top of
any nested hierarchical model.
...
particular once we have programs capable of specifying any nested multifactor
model it's straightforward to add the residual heterogeneity within the more
refined level by adding an explicit trivial factor.  This is one reason why
conditional heterogeneity is often ignored entirely in many references on
multifactor models.
...
$$
\begin{align*}
\delta_{i} &\sim \text{normal}(0, \tau)
\\
\delta_{ij} &\sim \text{normal}(0, \tau_{i})
\\
\theta_{ij} &= \theta + \delta_{i} + \delta_{ij}.
\end{align*}
$$
...
\theta_{lk}
&=
(\theta_{lk} - \theta_{l}) + \theta_{l} + 0,
\\
&=
(\theta_{lk} - \theta_{l}) + \theta_{l} + (-\theta + \theta),
\\
&=
(\theta_{lk} - \theta_{l}) + (\theta_{l} - \theta) + \theta
\\
&=
\delta_{lk} + \delta_{l} + \theta.
\end{align*}
$$
...
the behavior in any observed context, $\theta_{ij}$, we have to incorporate
_all_ of the contributions, including the baseline $\theta$, the first level
residuals $\delta_{i}$, the second level residuals $\delta_{ij}$, and so on.
Consequently none of these parameters have any interpretation independent of the
others.

We can continue to avoid confusion by always interpreting model inferences in
terms of explicit contexts, and the contributions from all of the factors levels
within which that context is nested.  Again those contexts can be actual, for
example constructed from observed levels and their inferred behavior, or
hypothetical, for example imagined levels where heterogeneity vanishes.  By
communicating all of the relevant details we avoid any interpretational
ambiguity.
...
becomes especially challenging.  In fact in this case we often appeal to models
...
color level, defining marginal and conditional exchangeabilities and motivating
a compatible hierarchical model.
...
not consistent with each other.  Permutations that are valid within the color
levels, for example, are obstructed by the symbol level categorization.
...
The only permutations that avoid crossing levels from both factors are those
restricted to the the same _intersection level_.
...
of intersections explodes multiplicatively.  Even if each factor seems
reasonable on its own the intersection factor can easily have hundreds if not
...
the more strongly those intersection parameters will be coupled together.  The
common behavior in all of the individual factor levels results in a tangled web
of complex interactions that can frustrate not only computation but also the
...
parameters.  For example consider the case of two factors, the first with levels
indexed by $l_{1} \in \{1, \ldots, L_{1}\}$, the second with levels indexed
$l_{2} \in \{1, \ldots, L_{2}\}$, and the corresponding intersection levels
indexed by the ordered pair $(l_{1}, l_{2})$.

One of the more frustrating but also critical aspects of these models is their
organization.  Here I'm trying to communicate that organization as clearly as
possible with a careful, explicit notation.  That notation, however, also
introduces a overwhelming mess of indices.  Hopefully the clarity is worth it!
...
$$
\begin{align*}
\theta^{1,2}_{l_{1}, l_{2}}
&=
\theta^{1,2}_{l_{1}, l_{2}} + 0 + 0
\\
&=
\theta^{1,2}_{l_{1}, l_{2}}
+ (-\theta^{1}_{l_{1}} + \theta^{2}_{l_{2}})
+ (-\theta^{1}_{j} + \theta^{2}_{l_{2}})
\\
&=
(\theta^{1,2}_{l_{1}, l_{2}} - \theta^{1}_{l_{1}} - \theta^{2}_{l_{2}})
+ \theta^{1}_{l_{1}} + \theta^{2}_{l_{2}}.
\end{align*}
$$
...
\\
&=
(\theta^{12}_{l_{1}, l_{2}} - \theta^{1}_{l_{1}} - \theta^{2}_{l_{2}} + \theta)
+ (\theta^{1}_{l_{1}} - \theta)
+ (\theta^{2}_{l_{2}} - \theta)
+ \theta
\\
&= \delta^{1,2}_{l_{1}, l_{2}} + \delta^{1}_{l_{1}} + \delta^{2}_{l_{2}} + \theta.
\end{align*}
$$

The baseline factor $\theta$ captures the behavior common to all contexts
while the residual factors $\delta^{1}_{l_{1}}$ and $\delta^{2}_{l_{2}}$ capture
the behavior common to the contexts within each individual factor level.  This
leaves the residual interaction factor, $\delta^{1, 2}_{l_{1}, l_{2}}$, which
captures any excess heterogeneity unique to the _overlap_ of the factors.

When these common behaviors are significant these residual parameters should be
less strongly coupled then the original intersection parameters.  Moreover even
if the intersection levels are sparsely populated the baseline and factor
...
to consider an artificial circumstance where the main symbol contributions and
all of the intersection contributions vanish.  If the artificiality of this
...
independent hierarchies to each contributing factor.  The advantage of this
construction over building a hierarchy over the main interaction factor levels
directly is that we have much more flexibility in the prior modeling.
...
may not be able to resolve those incompatibilities without massive data sets.
...
If these checks indicate that excess heterogeneity is important then we can add
the first-order residuals,
$$
\delta^{f_{1}, \ldots, f_{F}}_{l_{1}, \ldots, l_{F}}
\approx \theta + \sum_{f = 1}^{F} \delta^{f}_{l_{f}},
$$
and then checking for excess heterogeneity within the first-order intersection
...
within the scope of those observations.

Of course if that scope changes then the validity of a truncated model will
also, in general, change.  In particular a truncated model that is sufficient
for a small data set is unlikely to remain sufficient when we consider larger
data sets where we can resolve more and more subtle behaviors that often
manifest only in the higher-order interaction contributions.

Another limitation to expanding based on posterior retrodictive checks alone is
that we may not be able to design a summary statistic that is directly sensitive
to the potential heterogeneity.  This is especially true in complex models where
the heterogeneity is only one of many different, varying contributions to the
data generating process.  The residual scaffolding, however, is still useful
even without sensitive posterior retrodictive checks.  Because the incremental
model expansion compartmentalizes each order of the residuals we can fit and
analyze them one at a time.  Moreover we can motivate appropriate truncations
using the sequential posterior inferences, expanding the order of the residuals
until the posterior for the incremental contributions become consistent with
zero.
...
the $i$th index identifiesw hich element of the $i$th component vector
...
[Section 3.2](#sec:residual_expansion)!  In particular we can associate
$\mathbf{a}$ and $\mathbf{b}$ with two factors, $\mathbf{1} \otimes \mathbf{1}$
the rank zero baseline contribution, $\mathbf{a} \otimes \mathbf{1}$ and
$\mathbf{1} \otimes \mathbf{b}$ the rank one main contributions, and
$\mathbf{a} \otimes \mathbf{b}$ the rank two interaction contribution.
Truncating the residual expansion then corresponds to a _low rank approximation_
of the tensor product $(\mathbf{1} \oplus \mathbf{a} \oplus \mathbf{b})^{2}$.
...
In general given $I$ factors we can _systematically_ generate and organize all
of the terms in the residual expansion, including all of the higher-order
interaction factors, by expanding
...
critical information about how degenerate our inferences might be.
...
the different residual contributions.  For example if we observe only one
first-order intersection level then all of the interaction residuals associated
with the two component levels cannot be distinguished from the main residuals.
In order to be able to resolve each different interaction we need to observe
enough different contexts to differentiate between behavior common to the main
factors and residual heterogeneity particular to the interactions!

<center>
<br>
```{r, out.width = "50%", echo=FALSE}
knitr::include_graphics("figures/context_to_inter_degen/context_to_inter_degen.png")
```
<br><br>
</center>
...
## The Limitations of Multifactor Models
...
In particular how well a decomposition of heterogeneity across the factor levels
...
do not manifest as strong hetergeneity within demographic levels and their
lower-order interactions.  Consequently the choice of factors is critical to the
...
model will be able to represent.

At the same time the levels within each factor that are observed can be just as
important.  When observations exhaust all of the input factor levels we know
that our data are sensitive to the full range of possible behaviors that the
...
As critical as the choice of main factors, and the choice of observed levels in
those factors, is, we do not always have the luxury of choice in practice.  In
many cases the available factors are determined not by careful statistical
...
experiences common to all Black people.  Ignoring this interaction results in
not just poor inferences but also ineffectual policy informed by those
inferences.
...
Knowing that me might need flexibility we start with a general implementation
...
partition <- util$partition_div(fit)
div_samples <- partition[[1]]
nondiv_samples <- partition[[2]][1:1000,]
```
...
estimation.  In particular we'll compare the empirical variance of the empirical
means in each sufficiently occupied factor level to the empirical variance of
the empirical means of the sufficiently occupied factor levels, where a
sufficiently occupied level requires at least two observations.  Because the
factor heterogeneity should manifest directly in the mean of the observations
here this regularized $F$ statistic should be relatively sensitive to excess
heterogeneity.
...
unable to capturing not only the heterogeneity across the first-order
interaction levels but also the heterogeneity across the Main Factor 2 levels,
even though we've already added a hierarchy for this factor!
...
interaction factors all look much better, but the checks for the second-order
interaction factor suggests that we're not quite done.
...
Unsatisfied with just the first-order interactions we can now consider the
full residual expansion by adding the second-order interaction factor as well.
...
others.  When higher-order contributions are larger than the lower-order 
contributions adding higher-order residuals can give the lower-order residuals
freedom to explore different configurations, and unfortunately sometimes even
more computationally pathological configurations.

In any case our careful centering seems to have done the trick and we can move
onto posterior inferences.  First our posterior retrodicive checks which are
reasonable at all three orders.  While there is a mild deviation in Interaction 
Factor 123 it isn't particular worrisome given the heaviness of the posterior 
retrodictive distribution tail.
...
the true data generating process that we can resolve we can then consider
posterior inferences.  First let's look at the baseline parameter.
...
hist(samples$baseline, breaks=seq(-10, 20, 0.1), main="",
     col=c_dark, border=c_dark_highlight,
...
largely consistent with zero.  This in particular verifies that even if the 
deviation in the Interaction 123 posterior retrodictive check were significant
it would not be due to missing interactions.
...
We then continue to incorporating the observational model and fitting the
...
Now we can investigate the posterior retrodictive behavior.  No significant
deviations in across the main factor levels for any of the check times.
...
whether the factors, and the factor level assignments, are given or have to be
inferred from data.
...
hierarchical model that ignores the residual heterogeneity within the factor
levels.  Similarly the term "random effects model" denotes models with multiple
random effects and "mixed effects models" denotes models with some random
effects and some fixed effects, although neither term indicates _how_ the
effects are integrated together.  Better is the term "additive model" or more
precisely "additive mixed effects model" which indicates that multiple random
effects corresponding to different factors, including main factors and their
derived interactions, are added together as in the residual expansions of nested
multifactor model and overlapping multifactor models.  Morevoer the term
...
[GitHub](https://github.com/betanalpha/knitr_case_studies/tree/master/factor_models).
