...
How do we model the influence of some latent effect, $\gamma$, on the ordinal 
probabilities?  In particular how can we design our model such that a positive
effect, $\gamma > 0$ drags probability to higher ordinals while a negative 
effect, $\gamma < 0$, drags probability to lower ordinals?

Fortunately we can readily accomplish this moderation in our construction by 
translating $X$ upwards by the effect size, $x \mapsto x + \gamma$, while
keeping the cut points fixed.  Because of our prescient use of complementary
probability density functions and cumulative distribution functions above, a 
positive translation by $\gamma$ will correspond to probabilities 
concentrating towards higher ordinals as desired.
...
In the presence of a moderating effect the ordinal probabilities are computed as
$$
p_{k}
...
translating all of the interior cut points down by the effect size.  In other
words the ordinal probabilities inform only the _relative_ location of the
effect size and the cut points in this construction.  When the latent effect and 
...
we have to carefully anchor the effect size, the cut points, or perhaps even 
both to some base line values in the prior model.

An even better approach is to avoid modeling any baseline latent effect at all
and instead use this construction to model _heterogeneity_ in the latent effect
across varying circumstances.  For example consider a latent effect itself
...
for both the internal cut points and the latent effect $\gamma$ is given by
```{r}
...
points and the latent effect is clear from scatter plots of the posterior 
samples,
```{r}
c_light <- c("#DCBCBC")
...
Gaussian prior density function for the latent effect,
```{r}
...
develop a principle prior model we need to move away from the latent space and
...
probabilities into the $K - 1$ internal cut points and a latent effect $\gamma$ 
...
latent effect are non-identified and the map to probabilities is many-to-one.
...
$$
J_{k, 1} 
= \frac{ \partial S }{ \partial p_{k} }
= 1,
$$
a diagonal band,
$$
J_{k + 1, k + 1} 
= \frac{ \partial c_{k} }{ \partial p_{k + 1} }
= - \rho(c_{k}),
$$
