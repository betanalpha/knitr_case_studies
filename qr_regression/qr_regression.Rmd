...
## Interaction of the QR Decomposition and Priors
...
posterior?  The independent weakly informative priors we put on the nominal
slopes actually implies a strongly correlated prior for the transformed slopes!  

If we remove the weakly informative prior entirely,

```{r, comment=NA}
writeLines(readLines("qr_regr_no_prior.stan"))
```

```{r, cache=TRUE, warning=TRUE, message=TRUE, comment=NA}
qr_fit <- stan(file='qr_regr_no_prior.stan', data=input_data, seed=483892929)
```
...
then the posterior for the  transformed slopes becomes significantly less
correlated,
...
**Or it should.  What's going on here?**

In particular, the QR decomposition performs best when there the likelihood
dominates the prior, either due to sufficiently many data or sufficiently weak
prior information.
...
decomposition in the Stan manual.  The technique was originally introduced
into the Stan ecosystem by Ben Goodrich.
