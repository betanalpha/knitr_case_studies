...
theory will be devoid of any concrete examples.  This is not because of any
...
$$
Such sets are alwasy well defined even if the function does not have a unique
...
to a $\sigma$-algebra has little impact on the practical use of probability
theory.
...
closed in the topological sense are not exact opposites -- by definition the
...
$\sigma$-algebra on $X$ naturally defines a $\sigma$-algebra on $Y$ and
...
$A \in \mathcal{F}$ the conditional probability distribution becomes a measurable
...
specific a given probability distribution.  Many representations exploit the
particular structure of $X$ to fully characterize a probability distribution
...
$X$ needed to numerically compute a summation or integral is simply too
...
indicator functions.  In this way the sample procedure which is conceptually
capable of generating arbitrarily long sequences provides a faithful
representation of the corresponding probability distribution.  In practice we
...
will not in general be enough information to generate a sample from a conditional
...
If we can generate samples from the marginal probability distribution and the
...
a probability distribution over the observation space called the _true data
...
I warmly thank Maggie Lieu, Junpeng Lao, and Deen Abiola for helpful comments.
