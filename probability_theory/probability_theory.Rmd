...
Another important class of measurable functions are those for which
...
unlike projections these _doubly measurable_ transformations preserve all 
information encoded in a probability distribution.

In this case $(X, \mathcal{X}, \pi)$ and $(Y, \mathcal{Y}, \pi_{*})$ are just
two different manifestations, or _parameterizations_ of the same abstract 
system.  The two parameterizations, for example, might correspond to different 
choices of coordinate system, different choices of units, or different choices 
of language dialect capable of the same descriptions.  The doubly measurable 
transformations then serve as _reparameterizations_ that translate from one 
equivalent manifestation to another.
...
such as a probability mass function.  Reparameterizing our system, or more 
formally pushing our probability distribution forward along a sufficiently
well-behaved one-to-one transformation, yields a new manifestation of the same 
system but in the new space we have a different probability mass function, mean, 
and variance.
...
as naturally under a reparameterization.  The complication is that the
...
In particular, pushforward probability density functions pick up a determinant 
of the _Jacobian matrix of partial derivatives_,
...
If the reparameterization is _many-to-one_ then we have to take into account all
...
challenging and require analytically integrating a reparameterized probability 
density function over the complementary spaces.
...
reparameterization $g : x \mapsto x^{2}$?  
...
reparameterizations and so the pushforward of a probability mass function
is just the composition of the original probability mass function with the
reparameterization.  Lebesgue measures, however, are not necessarily invariant
under reparameterization; in general the pushforward of the Lebesgue
...
Now let's consider the reparameterization $g : x \mapsto x^{2}$.  With samples 
this transformation is straightforward to analyze by considering the pushforward
samples,
```{r}
pushforward_samples = sapply(stan_samples, function(x) x**2)
compute_mc_stats(pushforward_samples)
```
In particular we can readily study the reparameterized probability density 
...
Boris Shilov, and ssp3nc3r\@github for helpful comments as well as the feedback 
from those who have attended my courses.
