...
is used to model a variety of subtlety different systems, which then burdens the
...
confuse the reader, but rather is a consequence of the fact that we cannot
...
important results from set theory that we will need.  For a more complete and
rigorous treatment of set theory see @Folland:1999 and the appendix of
@Lee:2011.

A _set_ is a collection of elements with a _space_ the collection of all
elements under consideration.  For example, $\{1\}$, $\{5, 10, 12\}$, and
$\{30, 2, 7\}$, are all sets drawn from the space of natural numbers,
$\mathbb{N} = \left\{0, 1, \ldots \right\}$.  A _point set_ or _atomic set_ is
a set containing a single element such as $\{1\}$ above.  The entire space itself
is always a valid set, as is the _empty set_ or _null set_, $\emptyset$, which
...
$$
which reads "the set of elements $x$ in the space $X$ such that $f(x) = 0$."

## Set Operations

There are three natural operations between sets.

Given a set, $A$, its _compliment_, $A^{c}$, is defined by all of the elements
not in that set,
$$
...
elements included in either set,
$$
A_{1} \cup A_{2}  = \left\{ x \in X \mid x \in A_{1} \text{ OR } x \in A_{2} \right\}.
$$

Similarly, the _intersection_ of two sets, $A_{1} \cap A_{2}$, is defined as the
set of elements contained in both sets,
$$
...
intersection of any set and the empty set is just the empty set,
$A \cap \emptyset = \emptyset$.

## Functions

A _function_ is a relation that associates elements in one space to elements in
another space.  If a function, $f$, maps an element $x$ from the space $X$ to
the element $f(x)$ in the space $Y$ then we write
$$
\begin{alignat*}{6}
f :\; &X& &\rightarrow& \; &Y&
\\
...
the input space, $X$, by applying the inverse of a given function,
$$
...
manner.  In particular, we want our restriction to be _closed_ under the three
...
explicitly constructing a $\sigma$-algebra in practice, and the restriction of
the power set to a $\sigma$-algebra has little impact on the practical use of
probability theory.
...
_topologically open_ sets and the compliment of any open set is called a
...
are denoted _Borel $\sigma$-algebras_ [@Folland:1999].  Because common spaces
have natural topologies they consequently also have natural Borel
$\sigma$-algebras that exclude pathological sets that can cause problems.
...
and the transformations of that object.  To be fair, most of pure mathematics
also ultimately reduces to studying objects and their transformations!
Fortunately, the basic concepts of probability distributions and their
manipulations are reasonably intuitive, even from this more formal perspective.

Probability textbooks tend to be too simple, ignoring many important concepts
and succumbing to the pedagogical issues we have discussed, or focus on the
myriad technical details of probability theory and hence quickly fall beyond
the proficiency of many readers.  My favorite treatment of the more formal
...
From an abstract perspective, probability is a positive, conserved quantity
which we want to distribute across a space, $X$.  We take the total amount of
this conserved quantity to be 1 with arbitrary units, but the mathematical
consequences are the same regardless of this scaling.  From this perspective
probability is simply any abstract conserved quantity -- in particular it does
not refer to anything inherently random or uncertain.

A _probability distribution_ defines a mathematically self-consistent allocation
of this conserved quantity across $X$.  Letting $A$ be a sufficiently
well-defined subset of $X$, we write $\mathbb{P}_{\pi}[A]$ as the probability
assigned to $A$ by the probability distribution $\pi$.  Importantly, we want
this allocation to be self-consistent -- the allocation to any collection of
disjoint sets, $A_{n} \cap A_{m} = 0, n \ne m$, should be the same as the
allocation to the union of those sets,
$$
...
subsets of $X$, we conserve probability.
...
$$
In particular, this _countable additivity_ property allows us to cover complex
neighborhoods, such as that enclosed by a smooth surface, with an infinite
collection of sets and then calculate the probability allocated to that
neighborhood.
...
subsets of $X$.  Somewhat unnervingly we cannot construct an object that
self-consistently allocates probability to _every_ subset of $X$ because of
some very weird, pathological subsets.  Fortunately the same properties that
make these subsets pathological also prevent them from belonging to any
$\sigma$-algebra, consequently we can construct our probability distribution
relative to a given $\sigma$-algebra, $\mathcal{X}$.
...
x \sim \pi
$$
where $x \in X$ denotes the space, $\pi$ denotes the probability distribution,
...
number by averaging the function output at every point, $f(x)$, weighted by the
probability assigned around that point.  This weighting process emphasizes how
the function behaves in neighborhoods of high probability while diminishing its
behavior in neighborhoods of low probability.
...
\end{alignat*}
$$
For example this embedding associates the natural numbers,
$\left\{0, 1, 2, \ldots \right\}$, with the corresponding values in the real
line, or the interval $[0, 1]$ with the corresponding interval in the full
real line.
...
the real numbers, $X \subseteq \mathbb{R}^{N}$, then there is no natural
function whose expectation value defines a scalar mean.  We can, however, define
...
approximation.  In particular, because of countable additivity we can construct
a simple function bounded below by $f$ that approximates $f$ with arbitrary
accuracy.
...
Consequently we define the expectation of an everywhere-positive function as
...
different choices of units, or different choices of language capable of the same
...
distribution over a space onto a probability distribution over a
lower-dimensional subspace.  Let $\varpi: X \rightarrow Y$ be a _projection
operator_ that maps points in a space $X$ to points in the subspace
$Y \subset X$.  It turns out that in this case a $\sigma$-algebra on $X$
naturally defines a $\sigma$-algebra on $Y$ and the projection operator is
measurable with respect to this choice.  Consequently any _joint probability
distribution_ on $X$ will transform into a unique _marginal probability
distribution_ on $Y$.  More commonly we say that we _marginalize out_ the
complementary subspace, $Y^{C}$.
...
As we saw in [Section 2.4](#sec:measurable_transformations), projection
...
distribution on a subspace and construct a joint probability distribution on the
total space that projects back to the marginal?  We can if we can define an
appropriate probability distribution on the complement of the given subspace.
...
$A \in \mathcal{F}$ the conditional probability distribution becomes a
measurable function from $Y$ into $[0, 1]$ that quantifies how the probability
of that set varies as we move from one fiber to the next.
...
<br><br>
![](figures/disintegration/disintegration.png)
<br><br>

The induced joint distribution on the total space is consistent in the sense
that if we transform it back along the projection operator we recover the
...
case the fiber space is just $X$.
...
<br><br>
![](figures/product_disintegration/product_disintegration.png)
<br><br>

Conditional probability distributions are especially useful when we want to
construct a complex probability distribution over a high-dimensional space.
We can reduce the specification of the ungainly joint probability distribution
with a sequence of lower-dimensional conditional probability distributions and
marginal probability distributions about which we can more easily reason.  In
the context of modeling an observational process, this method of construction
a complicated distribution from intermediate conditional probability
distributions is known as _generative_ modeling.  In particular, each
intermediate conditional probability distribution models some fragment of the
full observational process.

# Representing Probability Distributions with Density Functions {#sec:density_representations}
...
calculations as needed instead of exhaustively specifying the relationships.
...
specific a given probability distribution.  In particular, _density
representations_ exploit the particular structure of $X$ to fully characterize
a probability distribution with functions.
...
across the natural numbers, $X = \left\{0, 1, 2, \ldots \right\}$, with the
...
In `R` we can evaluate both the exhaustive sum,
```{r}
K1 <- 5
K2 <- 10
...
is that in any non-atomic subset of $X$ there will be an uncountably infinite
number of points and the sum of probability mass function over that subset will
explode unless we assign zero probability mass to all but a countable number of
points.
...
as naturally along a measurable transformation on $X$.  Given a one-to-one
transformation $g : \mathbb{R}^{N} \rightarrow \mathbb{R}^{N}$, the differential
volumes over which we integrate can change, and probability density functions
have to change to compensate.  The probability density corresponding to the
pushforward distribution along a one-to-one transformation picks up a Jacobian
determinant,
$$
...
```
Note that I have not referred to this probability density function as
...
probability density function will pick up a Jacobian determinant that can skew
...
distribution except for the requirement that the total measure is 1.
Intuitively we can think of measures as allocations of _volume_ to well-defined
neighborhoods in a space.
...
natural only for the given coordinate system -- nonlinear transformations
between the real numbers change the coordinates, the volumes contained by
rectangles, and hence the Lebesgue measure.  Given a coordinate system,
however, the measure of any neighborhood in the real numbers and the expectation
of any function are given by the usual integral from calculus,
$$
...
when \$nu$ allocates zero volume only to those sets which $\mu$ also allocates
zero volume,
$$
...
$$
\mathbb{M}_{\nu} [ f ] = \mathbb{M}_{\mu} [ \lambda \cdot f ]
$$
...
into density function and a base measure, which in turn allows us to exploit
any computational properties of the base measure.  Probabilities and expectation
values of any probability distribution on a discrete space, for example, can
be computed with the summations,
$$
\begin{align*}
\mathbb{P}_{\pi} [ A ] &= \sum_{x \in A} \pi(x)
\\
\mathbb{E}_{\pi} [ f ] &= \sum_{x \in X} \pi(x) \,
\end{align*}
$$
where $\pi(x)$ is the density function,
$$
\pi(x) = \frac{ \mathrm{d} \pi }{ \mathrm{d} \chi }.
$$
...
notation and so I'm going to stick with it here.
...
where $\pi(x)$ is the density function,
$$
\pi(x) = \frac{ \mathrm{d} \pi }{ \mathrm{d} \mathcal{L} }.
$$
Comparing to the results in the past two sections we need that probability mass
...
of density functions.  Counting measures are invariant under
one-to-one transformations and the pushforward of a probability mass function
is just the composition of the original probability mass function with the
transformation.  Lebesgue measures, however, are not necessarily invariant
under one-to-one transformations.  In general the pushforward of the Lebesgue
measure require a Jacobian determinant to account for the compression or
expansion of volume in the transformation.  This change in the base measure
then induces an inverse Jacobian determinant in the probability density
function, as we saw in [Section 3.2](#sec:pdfs).
...
when trying to reasonable about well-posed computational algorithms.  For
example, the importance weights in importance sampling are given by the ratio
of the target probability density function to the importance probability
density function, which is actually the corresponding Radon-Nikodym derivative
and hence have well-defined values.
...
satisfy the asymptotic property, any mechanism for generating sequences will be
a valid sampling procedure.  The procedure can be generated algorithmically and
be completely deterministic given a known input configuration, generated from
unknown external inputs with unpredictable outputs, or anywhere in between.  It
also need not have any nice cryptographic properties or otherwise be hard to
...
distributions we can generate exact samples, but in general we typically have
...
guarantees that the empirical expectations are distributed around the true
expectations,
$$
\hat{f}(S_{N}) \sim
...
We can achieve the same samples using Stan [@Stan:2018] as well.  Here we
generate exact samples in the `generated quantities` block,
```{r}
...
second component corresponds to the next point, $x_{n + 1} \in X$, and the next
...
asymptotically with no guarantees on how they converge after only a finite
...
Under technical _ergodicity_ conditions, however, Markov chain Monte Carlo
estimators can achieve a central limit theorem of the form
$$
\hat{f}(S_{N}) \sim
\mathcal{N} \left( \mathbb{E}_{\pi} [ f ],
                   \sqrt{\frac{ \mathbb{V}[f] }{ N_{\mathrm{eff}}[f] } } \right).
$$
Here the _effective sample size_, $N_{\mathrm{eff}}[f]$, is a function of the
correlation between the points in the Markov chain and quantifies the size of an
exact sample that we would need to achieve the same accuracy in estimating
$\mathbb{E}_{\pi} [ f ]$.  As with the variance of $f$ we can estimate the
correlations needed to compute the effective sample size using the sample
generated by the Markov chain itself.
...
numbers.  These Markov chains achieve central limit theorems for many target
...
Monte Carlo see @Betancourt:2017a.

Target probability distributions in Stan are specified with probability density
functions defined in the Stan modeling language.  In a Stan program the latent
space is defined in the `parameters` block while the target density is defined
in the `model` block.  For example, a Stan program specifying a Gaussian density
is given by
```{r}
...
block which maps the states of the Markov chain through deterministic functions.
...
\end{alignat*}
$$
If the base space is also discrete with marginal probability mass function
...
\end{alignat*}
$$
If the base space is also continuous with marginal probability density function
$\pi_{X}(x)$ then a probability distribution over the total space can be
...
_mixed representation_ comprised as a product of a conditional probability
...
distribution over a space that contains _both_ discrete and continuous
...
projection operator at each point in the total sample.  A sample from the
total space, however, will not in general contain enough information to
construct a sample from a conditional probability distribution.  The problem is
that in most cases each fiber will be allocated a small or outright zero
probability by the total probability distribution and no samples will fall
exactly on a given fiber.  Instead we have to generate samples from a
conditional probability distribution by first evaluating it at a particular
point in the base space and then generating samples from the corresponding
probability distribution.

For the example in [Section 5.2](#sec:conditional_densities) this would take
the form
...
We can achieve the same samples using Stan using the `generated quantities`
block and the `fixed_param` sampler,
```{r}
...
If we know the conditional probability density function explicitly then we can
write a Stan program that targets it directly using the `data` and
`transformed data` blocks to define the values of the conditioning variables.
...
often be intractable.  Fortunately we can take advantage of the fact that
the conditional probability density function is _proportional_ to the joint
probability density function with the conditioning parameters set to their
known values,
$$
\pi(x_2 \mid x_1 = x) \propto \pi(x_1 = x, x_2).
$$
Consequently we can we can write a Stan program that implements the joint
distribution in the `model` block and defines indicates the conditioning by
defining $x_{1}$ in the `transformed data` block and $x_{2}$ in the
`parameters` block.
```{r}
...
for complex conditional probability distributions.

If we can generate samples from a marginal probability distribution and a
conditional probability distribution then we can immediately generate samples
from the corresponding joint probability distribution on the total space.  We
simply generate a sample from the marginal distribution and then generate
samples from the conditional probability distribution evaluated at each point in
the marginal sample.  This _ancestral sampling_ technique is particularly useful
when we have constructed the total probability distribution generatively out of
many simple intermediate probability distributions.
...
_ontological_ randomness that is inherent to the world or _epistemological_
...
$$
with the data set to their measured values, as discussed in
[Section 5.4](#sec:conditional_samples).

# Conclusion

Probability theory is a powerful tool for both modeling, and making inferences
about, the world around us.  Often that power is undercut by confusion in
exactly what objects and computations probability theory defines, and how we can
...
we wield it correctly.

# Acknowlegements

I warmly thank Maggie Lieu, Junpeng Lao, Deen Abiola, Jeremy Mason, Victor
Salgado, xcurry\@github, and Sean Talts for helpful comments.
