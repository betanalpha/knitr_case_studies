...
$$
\begin{align*}
B_{1} &= \left\{ x \in \mathbb{N} \mid -2 \le x \le 1 \right\}
\\
B_{1} &= \left\{ x \in \mathbb{N} \mid 0 \le x \le \right\}
\end{align*}
$$
...
$$
B_{1}^{c} = \left\{ x \in \mathbb{N} \mid x < -2 \text{ OR } x > 1 \right\}.
$$
...
constructed.  In particular, one can show that these non-constructable sets
satisfy some very odd properties.
...
many of the pathologies behaviors that arise in uncountably-large spaces.
...
non-constructible sets is critical to a well-defined probability theory 
$\sigma$-algebras will be a common occurance.
...
of this conserved quantity across $X$.  If $A$ subset of $X$ then we write
...
$X$.  In other words we requires that the probability assigned to the full space
...
x \sim \pi
$$
where $x \in X$ is an arbitrary point denoting the space, $\pi$ denotes the 
probability distribution, and a valid $\sigma$-algebra is just assumed.  Care 
must be taken to not over interpret this notation -- in particular it says 
nothing about individual points $x \in X$ but rather denotes to a probability 
...
Conveniently the distribution of probability across a space has provides a 
...
characterize the given probability distribution itself.  In this section we'll 
be introduced to expectations of certain functions that are particularly useful 
in interrogating the properties of probability distributions defined on certain 
spaces. 

First, however, a word of caution.  The interpretation of the specific 
expectation values of these special functions is specific to the context of a
given space and will not, in general, push forward along a transformation to
another space.  To ensure a meaningful expectation value we must first push the
special function forward along the transformation and only then perform an
expectation.  Expectation and transformation typically do not commute!
...
\mathbb{c}_{\pi, n} 
\equiv 
\mathbb{E}_{\pi} [ (\iota - \mathbb{m}_{\pi, 0})^{n} ].
$$
...
infinite or simply ill-posed.  One the other hand the embedding function itself 
...
$$
\mathbb{E}_{\pi_{*}} [ B ] = \sum_{x \in X } \pi(x) f(g(x)).
$$
...
It's taken quite the buildup, but we now finally ready compute our first 
...
forward along a one-to-one transformation yields a different perspective of the
same system but in the new space we have a different probability mass function,
mean, and variance.

Using the transformation rules of a probability mass function introduced above
we can visualize the probability mass function of the pushforward distribution,
```{r}
...
stabilizing transformation_ for distributions that admit Poissian probability
mass functions.

Transformations like these are often used to simplify the use of a given 
...
as naturally along a measurable transformation.  The complication is that the
...
In particular,
pushforward probability density functions pick up a determinant of the 
_Jacobian matrix of partial derivatives_,

For example, the pushforward probability density function corresponding to a 
_one-to-one_ transformation $g : \mathbb{R}^{N} \rightarrow \mathbb{R}^{N}$ 
picks up an inverse factor of the Jacobian determinent,
$$
\begin{align*}
\pi_{*} (y) 
&= \pi(g^{-1}(y)) |J(g^{-1}(x))|^{-1}
...
If the transformation is _many-to-one_ then we have to take into account all
of the roots of $y = g(x)$ for a given y, 
...
Transformations that change the dimensionality of the space, such as 
marginalizations, are more challenging and require analytically integrating
out the complementary spaces out of a pushforward probability density function.
...
How do we comput the probability allocated to the interval $B_{1}$ introduced
...
The real line have a unique embedding into the real line -- the identify 
...
How does a Gaussian probability density function change under the one-to-one
transformation $g : x \mapsto x^{2}$?  

The first step in constructing the pushforwards is computing the inverse of the 
transformation,
```{r}
y_inv <- function(y) {
  return(sqrt(y))
...
plot(1, type="n", xlim=c(0, 16), xlab="dy", ylim=c(0, 1), ylab="",
     cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5, yaxt='n')
for (x in delta_x) abline(v=x * x, col=c_dark, lwd=2)
```
...
Gaussian probability density functions on certain spaces, $X = \mathbb{R}$.

Because Gaussian probability density functions are particular convenient to work
with there is a great deal of literature in engineering transformations that can
push a given probability density function into something more Gaussian.  In
particular the _Box-Cox_ family of transformations,
$$
g(x) = \frac{ x^{\phi} - 1 }{ \phi },
$$
are commonly used to massage a given space and the probability density function
it admits into something like is reasonably well-approximated with a Gaussian
...
and hence has well-defined values.
...
Finally we can use our samples to quantity the pushforward distribution along 
$g : x \mapsto \sqrt{x}$.  The mean, for example, is estimated as
```{r}
...
xs <- rep(os, each=2)[2:1999]
ys <- rep(ecdfs, each=2)[2:1999]
lines(rep(os, each=2)[2:1999], rep(ecdfs, each=2)[2:1999],
      col=c_mid_highlight, lwd=2)

os <- sort(stan_samples)
ecdfs <- sapply(os, function(x) length(stan_samples[stan_samples <= x])) / length(stan_samples)
xs <- rep(os, each=2)[2:1999]
ys <- rep(ecdfs, each=2)[2:1999]
lines(rep(os, each=2)[2:1999], rep(ecdfs, each=2)[2:1999], type="l",
      col=c_light_highlight, lwd=2)
```
...
Now let's consider the transformation $g : x \mapsto x^{2}$.  With samples this
...
compute_mc_stats(pushforward_samples)
```
In particular we can readily study the pushforward probability density function
by histogramming the pushforward samples,
```{r}
ys <- seq(0, 25, 0.001)
pdfs <- sapply(ys, function(y) dnorm(y_inv(y), mu, sigma) * abs_J(y))
...
Salgado, xcurry\@github, and Sean Talts for helpful comments as well as the 
feedback from those who have attended my courses.
...
Simpson, David Pascall, David Roher, Elad Berkman, Fernando Parra, Finn 
Lindgren, Granville Matheson, Hernan Bruno, J Michael Burgess, Joel Kronander,
Jonas Beltoft Gehrlein, Joshua Mayer, Justin Bois, Lars Barquist, Marek 
Kwiatkowski, Maurits van der Meer, Maxim Kesin, Michael Dillon, Ole Rogeberg,
Oscar Olvera, Riccardo Fusaroli, Richard Torkar, Sam Petulla, Sam Zorowitz,
Seo-young Kim, Seth Axen, Shoshana Vasserman, Simon Duane, Stephen Oates,
Stijn, and Vladislavs Dovgalecs.
